...
class AsyncScraper:
    def __init__(self, start_url, matched_urls_file='matched_urls.txt', processed_urls_file='processed_urls.txt'):
        ...
        # Concurrency control
        self.semaphore = asyncio.Semaphore(100)  # Adjust based on your system and the server's capacity
    ...
    async def scrape(self):
        ...
        connector = aiohttp.TCPConnector(limit=0)  # Remove the limit on the number of connections
        timeout = aiohttp.ClientTimeout(total=60)
        async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
            tasks = []
            num_workers = 100  # Adjust based on your system and the server's capacity
...
if __name__ == "__main__":
    scraper = AsyncScraper(start_url="http://breistest.lexiconista.com/en/gram/blasta")
